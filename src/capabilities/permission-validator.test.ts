...
describe("validatePermissions", () => {
  it("returns empty findings for empty permissions", () => {
    const findings = validatePermissions({});
    expect(findings).toEqual([]);
  });

  it("flags dangerous filesystem write path /", () => {
    const findings = validatePermissions({ filesystem: { write: ["/"] } });
  ...
  it("flags wildcard shell allowed commands", () => {
    const findings = validatePermissions({ shell: { allowedCommands: ["*"] } });
  ...
  it("flags wildcard network allowed hosts", () => {
    const findings = validatePermissions({ network: { allowedHosts: ["*"] } });
  ...
  it("flags wildcard secrets env var", () => {
    const findings = validatePermissions({ secrets: { requiredEnvironmentVariables: ["*"] } });
  ...
  it("flags memory write without promotion approval", () => {
    const findings = validatePermissions({ memory: { canWrite: true, requiresPromotionApproval: false } });
