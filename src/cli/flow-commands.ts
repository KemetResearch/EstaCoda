...
export async function flowCommand(options: CliOptions, args: string[]): Promise<CliCommandResult> {
  ...
  try {
    switch (subcommand) {
      case "list":
        return await flowList(db);
      case "show":
        return await flowShow(db, restArgs);
      case "status":
        return await flowStatus(db, restArgs);
      case "trace":
        return await flowTrace(db, restArgs);
      case "pause":
        return await flowPause(db, restArgs);
      case "resume":
        return await flowResume(db, restArgs);
      case "interrupt":
        return await flowInterrupt(db, restArgs);
      case "cancel":
        return await flowCancel(db, restArgs);
      case "steer":
        return await flowSteer(db, restArgs);
      case "approve":
        return await flowApprove(db, restArgs);
      case "reject":
        return await flowReject(db, restArgs);
      case "retry":
        return await flowRetry(db, restArgs);
      case "skip":
        return await flowSkip(db, restArgs);
      case "checkpoint":
        return await flowCheckpoint(db, restArgs);
      case "compact":
        return await flowCompact(db, restArgs);
      case undefined:
      case "help":
      case "--help":
      case "-h":
...
async function flowList(db: SQLiteSessionDB): Promise<CliCommandResult> {
  const { store } = createFlowServices(db);
...
async function flowShow(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
  if (flowId === undefined) {
...
async function flowStatus(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
  if (flowId === undefined) {
    return { handled: true, exitCode: 1, output: "Usage: estacoda flow status <flowId>" };
  }

  const { dispatcher } = createFlowServices(db);
...
async function flowTrace(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
...
async function flowPause(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
  if (flowId === undefined) {
...
async function flowResume(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
  if (flowId === undefined) {
...
async function flowInterrupt(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
  if (flowId === undefined) {
...
async function flowCancel(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
  if (flowId === undefined) {
...
async function flowSteer(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
...
async function flowApprove(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const stepId = args[0];
  if (stepId === undefined) {
...
async function flowReject(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const stepId = args[0];
  if (stepId === undefined) {
...
async function flowRetry(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const stepId = args[0];
  if (stepId === undefined) {
...
async function flowSkip(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const stepId = args[0];
  if (stepId === undefined) {
...
async function flowCheckpoint(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
...
async function flowCompact(db: SQLiteSessionDB, args: string[]): Promise<CliCommandResult> {
  const flowId = args[0];
  if (flowId === undefined) {
