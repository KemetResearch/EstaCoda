...
describe("semantic compression deterministic evals", () => {
  it("preserves core coding task state from a long realistic transcript", async () => {
    ...
    const harness = auxiliaryHarness([
      ...
      "- Do not change TaskFlow compaction.",
      "## Completed Actions",
...
function realisticCodingTranscript(): SessionMessage[] {
  return [
    ...
    msg("constraint", "user", "Constraint: keep compression gated by experimental config and do not change TaskFlow compaction. ".repeat(4), { explicitConstraint: true }),
