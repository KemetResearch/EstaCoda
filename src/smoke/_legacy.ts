...
assert(gatewayStatusLocked.exitCode === 0, "expected gateway status to succeed even without token");
assert(gatewayStatusLocked.output.includes("EstaCoda gateway status"), "expected gateway status header");
assert(gatewayStatusLocked.output.includes("Channels"), "expected gateway status channels section");
assert(gatewayStatusLocked.output.includes("Telegram: configured, missing credentials"), "expected gateway status shows telegram missing credentials");
