...
export const workflowLockingCase: EvalCase = {
  id: "workflow-locking",
  name: "Workflow run lock acquire, release, heartbeat, and stale recovery",
  description: "Deterministic lock lifecycle with lease expiration and recovery.",
  tags: ["workflow", "locking", "deterministic"],
  run: async (): Promise<EvalResult> => {
    ...
    const afterRecovery = await lockService.get("run-1");
    assertions.push(assertTrue("lock removed after stale recovery", afterRecovery === null));

    // Acquire after recovery
    const postRecovery = await lockService.acquire("run-1", "owner-c");
    assertions.push(assertTrue("acquire after stale recovery succeeds", postRecovery));

    // Release by wrong owner does nothing
    await lockService.release("run-1", "owner-x");
    const stillThere = await lockService.get("run-1");
    assertions.push(assertTrue("release by wrong owner preserves lock", stillThere !== null));

    return buildResult("workflow-locking", "Workflow run lock acquire, release, heartbeat, and stale recovery", assertions, Date.now() - startedAt);
  }
};
