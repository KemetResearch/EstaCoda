...
export const memorySafetyFilesProtectedCase: EvalCase = {
  ...
  description: "SOUL.md and AGENTS.md entries are always rendered and cannot be deactivated.",
  ...
  run: async (): Promise<EvalResult> => {
    ...
    // So SOUL.md/AGENTS.md entries are not in the promotion store per se.
    ...
    //    the protection is actually at the renderer level (always include SOUL.md/AGENTS.md)
    ...
    const containsAgents = render.text.includes("Follow the plan.");

    const assertions = [
      assertTrue("SOUL.md is safety file", isSoulSafety),
      assertTrue("AGENTS.md is safety file", isAgentsSafety),
      ...
      assertTrue("renderer includes AGENTS.md even with no-match query", containsAgents)
    ];
