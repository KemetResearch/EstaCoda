...
| 14 | `setupEditor.shell.stateLabels` | `src/onboarding/config-editor/render.ts` | `State:`, `kind`, `route`, `editor mode`, `recommended`, `model`, `user config`, `project config` | Hardcoded labels include paths and IDs without Arabic isolation path. | State, route, editor mode, recommended action, model, user config, project config | الحالة، المسار، وضع المحرر، الإجراء المقترح، النموذج، إعداد المستخدم، إعداد المشروع | state kind, route kind, provider/model ID, config paths | guided editor shell | required | Register labels as a group; values must remain isolated. |
