...
Edge TTS package installation is a separate local operator consent boundary. Setup Editor and onboarding may install the global managed Python capability only after the reviewed setup plan is confirmed; direct interactive `estacoda voice setup --tts-provider edge` may offer installation after explicit confirmation. Non-interactive voice setup, runtime voice synthesis, and gateway auto-TTS do not install packages. Remote gateway messages must only receive or log the repair path when the capability is missing.
