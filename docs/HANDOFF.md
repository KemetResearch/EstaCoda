...
- Voice configuration foundation now has Hermes-aligned `tts`/`stt` config plus `estacoda voice status/setup` and `settings voice`; actual synthesis/transcription tools remain next. `smoke-tested`
