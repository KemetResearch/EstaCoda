...
- Voice configuration foundation now has Hermes-aligned `tts`/`stt` config plus `estacoda voice status/setup` and `settings voice`; OpenAI-compatible `voice.speak` writes audio-cache artifacts, OpenAI/Groq-style `voice.transcribe` writes transcript artifacts, and Telegram uploads audio artifacts with `sendAudio`. `smoke-tested`
