...
{
  "name": "manim-video",
  "description": "Create mathematical, algorithmic, and technical explainer videos with Manim Community Edition.",
  "version": "1.0.0",
  "category": "creative",
  "origin": {
  ...
  "whenToUse": [
    "The user wants animated explanations, math animations, or concept visualizations.",
    "The user wants algorithm walkthroughs or technical explainers.",
    "The user wants 3Blue1Brown-style educational video output."
  ],
  "requiredToolsets": [
    "files",
    "shell-write"
  ...
  "playbook": [
    {
      "id": "plan-scenes",
      "description": "Draft the narrative arc, scene list, visual language, formulas, diagrams, and render plan.",
      "toolsets": [
        "files"
      ],
      "successCriteria": [
        "A scene plan or equivalent structure exists."
      ]
    },
    {
      "id": "write-manim-script",
      "description": "Author or update the Manim Python script with independently renderable scenes.",
      "toolsets": [
        "files",
        "coding"
      ],
      "successCriteria": [
        "The Manim script contains the requested scenes and visual elements."
      ]
    },
    {
      "id": "render-review",
      "description": "Run setup checks and Manim/ffmpeg render commands when appropriate, then report artifacts and issues.",
      "toolsets": [
        "shell-write"
      ],
      "successCriteria": [
        "Render or check results are reported with output paths or next fixes."
      ]
    }
  ],
  ...
  "examples": [
    "Create a manim animation explaining Fourier series.",
    "Visualize quicksort with manim.",
    "Make a 3Blue1Brown-style video about eigenvalues."
  ],
  "evaluations": [
    {
      "input": "Create a manim animation explaining Fourier series",
      "shouldUseToolsets": [
        "files",
        "shell-write"
      ],
      "expectedOutcome": "The agent writes a Manim script and renders or validates video output."
    }
  ]
}
---
...
Deploy this skill when the user requests animated explanations, mathematical visualizations, algorithm walkthroughs, equation derivations, architecture diagrams, or data stories. The output is 3Blue1Brown-style educational video rendered through Manim Community Edition.

## Quality Bar

Animation carries the pedagogy. Every scene must advance understanding, not merely decorate it.

Lock the narrative before the first line of code. What error does this dismantle? Where does the insight crystallize? Treat the user's prompt as raw material — shape it into a visual argument.

**Show the picture before the proof.** A geometric pattern seen before its algebraic form lodges deeper in memory. The formula should arrive as confirmation, not revelation.

**Single-pass excellence.** The first render must be legible, coherent, and visually unified. Clutter, bad timing, or "slide-deck" aesthetics are failures.

**Layered opacity directs the eye.** Full strength for what matters now. Half strength for context. Barely visible for structure. The viewer's attention is a scarce resource — budget it.

**Silence is part of the score.** `self.wait()` after every animation. Absorption needs time. Two seconds of stillness after a key moment pays for itself.

**Visual discipline across scenes.** One palette, one type scale, one motion grammar. Technical correctness with chromatic chaos is still a failure.

## Prerequisites

Run `scripts/setup.sh` to verify dependencies. Requires Python 3.10+, Manim CE v0.20+ (`pip install manim`), LaTeX (`texlive-full` on Linux, `mactex` on macOS), and ffmpeg. Reference docs are tested against Manim CE v0.20.1.
...
One Python script per project. No browser, no Node.js, no GPU required.
...
1. **PLAN** — Draft `plan.md`: narrative arc, scene list, visual elements, palette, voiceover script
2. **CODE** — Author `script.py` with one class per independently renderable scene
3. **RENDER** — `manim -ql script.py Scene1 Scene2 ...` for draft, `-qh` for final
4. **STITCH** — ffmpeg concat of scene clips into `final.mp4`
5. **AUDIO** (optional) — Layer voiceover and/or music via ffmpeg. See `references/rendering.md`
6. **REVIEW** — Generate preview stills, cross-check against plan, iterate
