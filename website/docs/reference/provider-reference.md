...
| **Local / Custom Endpoint** | `implemented` | Built-in `local` provider for Ollama, LM Studio, llama.cpp, vLLM, LiteLLM, or another OpenAI-compatible local/custom endpoint. Defaults to `http://localhost:11434/v1`, `authMethod: "none"`, and optional API key env `OPENAI_COMPATIBLE_API_KEY`. |
