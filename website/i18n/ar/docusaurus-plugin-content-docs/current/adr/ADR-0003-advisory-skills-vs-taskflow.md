---
title: ADR-0003 المهارات الاستشارية مقابل TaskFlow
description: المهارات الاستشارية المبنية على Markdown أولًا والتنظيم المُنفذ الدائم عبر TaskFlow.
sidebar_position: 3
---
...
- v0.8 يُدخل TaskFlow للتنظيم المُنفذ الدائم.
...
- تُنفذ سير العمل المُنفذة عبر TaskFlow، الذي يسجل كل خطوة، ويُنفذ الانتقالات، ويحظر التغييرات غير القانونية.
...
- `estacoda flow` — أوامر مشغل TaskFlow
- `src/skills/skill-loader.ts` — تحليل وتحقق المهارات
- `src/taskflow/` — محرك التنظيم الدائم
...
- يجب أن تبقى انتقالات حالة TaskFlow صارمة. الانتقالات غير القانونية تُطلق `IllegalTransitionError`؛ تخفيف هذا يُفسد ضمانات التنفيذ.
...
- تكوين سير العمل عبر المهارات. التدفق ينتمي إلى مهارة واحدة أو طبقة تكوين صريحة.

## صفحات ذات صلة

- [المهارات](../user-guide/skills.md)
- [TaskFlow CLI](../reference/cli-commands.md)
- [ADR-0006: آلة حالة TaskFlow](./ADR-0006-taskflow-state-machine.md)
