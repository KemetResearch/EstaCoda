...
إذا كانت نقطة النهاية تتطلب مفتاحًا، خزّن المفتاح الخام في ملف `.env` للملف الشخصي المختار وأشر إليه من الإعداد:
...
استخدم معرف مزود مخصص فقط عندما تحتاج هوية مسار OpenAI-compatible منفصلة:
...
يسأل الإعداد التفاعلي عن مزود TTS فقط. يستخدم الإعدادات الافتراضية للتشغيل للنماذج والأصوات وإعدادات المزود، ويجمع مفاتيح API للمزودين المستضافين عبر إدخال masked. تخزن الإعدادات مراجع متغيرات البيئة مثل `apiKeyEnv`، بينما تُكتب قيم الأسرار المحلية للملف الشخصي إلى `.env` فقط بعد review/apply. تبقى أعلام CLI المباشرة للمشغل مثل `estacoda voice setup --tts-model` و`--tts-api-key-env` و`--tts-api-key` متاحة للإعداد عبر السكربتات.
...
يسأل الإعداد التفاعلي عن مزود STT فقط. يستخدم الإعدادات الافتراضية للتشغيل للنماذج وإعدادات المزود، ويجمع مفاتيح API للمزودين المستضافين عبر إدخال masked. تخزن الإعدادات مراجع متغيرات البيئة مثل `apiKeyEnv`، بينما تُكتب قيم الأسرار المحلية للملف الشخصي إلى `.env` فقط بعد review/apply. تبقى أعلام CLI المباشرة للمشغل مثل `estacoda voice setup --stt-model` و`--stt-api-key-env` و`--stt-api-key` متاحة للإعداد عبر السكربتات.
...
`skills.autonomy` هو مفتاح التوافق المحفوظ لـ Agent Evolution. يتحكم في سلوك الأدلة وproposal القابل للمراجعة. وضع `autonomous` هو وضع ظل فقط: يسجّل القرارات للمراجعة ولا ينفذ ترقية تلقائية أو استرجاعًا تلقائيًا أو تعديلًا لملفات المهارات.
...
- تكتب عمليات إعداد المزود مفاتيح API الخام فقط إلى `.env` الخاص بالملف الشخصي المحدد، وليس إلى `config.json`. نقاط النهاية المحلية / الخاصة بلا اعتماد افتراضيًا، ولا تكتب مفتاحًا إلا عندما يقدمه المستخدم.
- تعرض review manifests لإعداد Voice مراجع متغيرات البيئة فقط. لا تُخزن مفاتيح API الخام الخاصة بـ Voice في الإعدادات، ولا تظهر في review manifests، ولا تُدرج في prompt context.
