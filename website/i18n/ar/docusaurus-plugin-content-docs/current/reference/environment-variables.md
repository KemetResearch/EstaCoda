...
| `OPENAI_COMPATIBLE_API_KEY` | نقطة نهاية محلية / مخصصة متوافقة مع OpenAI | اختياري لـ `local`؛ افتراضي لمسارات OpenAI-compatible المخصصة |
...
تخزن إعدادات Voice مراجع متغيرات البيئة فقط. يجمع Setup Editor وOnboarding Wizard مفاتيح API لمزودي Voice المستضافين عبر إدخال masked، ويكتبان القيم الحقيقية إلى `.env` الخاص بالملف الشخصي المحدد فقط بعد review/apply. لا تُخزن مفاتيح API الخام الخاصة بـ Voice في الإعدادات، ولا تظهر في review manifests، ولا تُدرج في prompt context. توفر إعدادات التشغيل الافتراضية نماذج Voice والأصوات وإعدادات المزود، لذلك لا يطلب الإعداد التفاعلي model strings أو أسماء مراجع متغيرات البيئة.

مزودو Voice بلا مفتاح هم STT `local` وTTS `edge`. لا يتطلب `edge` مفتاح API، لكنه شبكي: يُرسل نص التوليف إلى خدمة Microsoft Edge speech وليس محليًا/offline. المزودون المستضافون الذين يتطلبون مفتاحًا هم STT `openai` و`groq` و`xai`؛ وTTS `openai` و`elevenlabs` و`minimax` و`gemini` و`xai`. المزودون المؤجلون/الـ stubs هم STT `mistral` وTTS `mistral` و`neutts` و`kittentts`. يمكن إعادة استخدام بيانات اعتماد موجودة عبر مراجع متغيرات البيئة مثل `VOICE_TOOLS_OPENAI_KEY` و`OPENAI_API_KEY` و`GEMINI_API_KEY` و`GROQ_API_KEY` و`XAI_API_KEY`، وعبر إعدادات/مسارات المزود المتوافقة الموجودة حيثما يكون ذلك مدعومًا. لا توجد مجموعات بيانات اعتماد صوتية أو وسطاء بوابة أو بدائل مدارة أو مصادر غير بيئية.
...
| `ARK_API_KEY` | أمثلة BytePlus / Seedream؛ يعيد الإعداد المراجع استخدامه عندما يكون موجودًا |
