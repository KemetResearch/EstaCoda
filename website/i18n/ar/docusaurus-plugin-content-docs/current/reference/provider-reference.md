...
| `configurable` | يظهر في الإعداد/اختيار النموذج ويمتلك دعم محوّل قابل للتشغيل، لكنه قد لا يكون مثبتاً عملياً. | اعدّده إذا كنت تختبر أو تُرسِل. |
...
تستخدم مسارات LLM الرئيسية والاحتياطية مهلة كلية ومهلة ركود/انعدام تقدم:

| الحقل | الافتراضي | ملاحظات |
|---|---:|---|
| `timeoutMs` | `1800000` | ميزانية طلب المزود الكلية. |
| `staleTimeoutMs` | `120000` | مدة انتظار رؤوس الاستجابة في الطلبات غير المتدفقة؛ والمدة بين البايتات المستلمة في الطلبات المتدفقة. |

الأسبقية خاصة بكل مسار: قيمة `model` أو مسار احتياطي، ثم قيمة `providers.<id>`، ثم الافتراضي المدمج. تحتفظ مسارات النماذج المساعدة بسلوك `auxiliaryModels.*.timeoutMs` الحالي ولا تحصل على ضوابط `staleTimeoutMs` في هذا التنفيذ. لا تعرض أوامر إعداد CLI أعلامًا لمهلات المزود؛ اضبط هذه القيم في إعدادات الملف الشخصي المحدد.
...
طلبات `modelOverride` للأطفال المفوضين تستخدم مسارات مزودين مضبوطة فقط. التجاوزات على المزود نفسه وعبر مزود آخر مراجع تحفظ إعداد المزود الهدف، وتستخدم بيانات اعتماد `apiKeyEnv` الموجودة، وتحترم `authMethod: "none"`، وترفض `enableNetwork: false` قبل التنفيذ، وتعطل fallback routes للطفل المتجاوز. لا تضيف credential pools ولا تعدّل مسارات الأب/session/profile.
