...
يجب ألا تُحفظ command buffers كنص مستخدم في transcript أو history. الكتابة العادية أثناء الدور النشط تُتجاهل ما لم تبدأ أمرًا بـ `/`؛ وحتى عندها، نص الأمر هو إدخال تحكم وليس رسالة مستخدم. يجب أن يبقى transcript المرئي ممثلًا للنص الأصلي الذي أرسله المستخدم، بينما يظهر نص إعادة المحاولة مع marker صريح عند استخدام `/steer`.

مطالبات الأسرار خارج مسار chrome هذا. يجب ألا يظهر المحتوى الملصوق في مطالبة سرية ضمن paste preview أو transient bottom chrome أو تلميحات slash أو status rails أو السجلات أو رسائل command lane.
